...
def create_new_shipment(shipment: ShipmentCreate, db: Session = Depends(get_db)):
    return create_shipment(db=db, shipment=shipment)

@router.get("/", response_model=List[Shipment])
def read_shipments(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
...
def update_existing_shipment(shipment_id: int, shipment: ShipmentUpdate, db: Session = Depends(get_db)):
    updated_shipment = update_shipment(db=db, shipment_id=shipment_id, shipment=shipment)
